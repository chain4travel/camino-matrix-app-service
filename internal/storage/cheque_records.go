...
func (s *storage) prepareChequeRecordsStmts(ctx context.Context) error {
	...
	getChequeRecordsWithPendingTxs, err := s.db.PreparexContext(ctx, fmt.Sprintf(`
		SELECT * FROM %s
		WHERE status = %d
	`, chequeRecordsTableName, models.ChequeTxStatusProcessing))
	if err != nil {
		s.logger.Error(err)
		return err
	}
	...
	getChequeRecordByTxID, err := s.db.PreparexContext(ctx, fmt.Sprintf(`
		SELECT * FROM %s
		WHERE tx_id = ?
	`, chequeRecordsTableName))
	if err != nil {
	...
	s.getChequeRecordByTxID = getChequeRecordByTxID
